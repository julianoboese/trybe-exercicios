...
describe('01 - Testa mockagem de funções', () => {
  // Exercício 1
...
// Exercício 5
describe('02 - Testa mockagem de função assíncrona', () => {
  service.fetchDogImage = jest.fn();
  afterEach(service.fetchDogImage.mockReset);

  test('Testa função assíncrona fetchDogImage resolvida', async () => {
  ...
  test('Testa função assíncrona fetchDogImage rejeitada', async () => {
    service.fetchDogImage.mockRejectedValue('request failed');

    expect(service.fetchDogImage).not.toHaveBeenCalled();
    await expect(service.fetchDogImage()).rejects.toBe('request failed');
